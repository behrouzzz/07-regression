...
* How can one generalize simple linear regression (SLR) to incoporate lots of regressors for the purpose of prediction?
...
* Least squares (and hence maximum likelihood (ML) estimates under iid Gaussianity of the errors) minimizes
$$
...
sum(ey * ex2) / sum(ex2 ^ 2)
coef(lm(ey ~ ex2 - 1))

ey = resid(lm(y ~ x + x2))
ex3 = resid(lm(x3 ~ x + x2))
sum(ey * ex3) / sum(ex3 ^ 2)
coef(lm(ey ~ ex3 - 1))
