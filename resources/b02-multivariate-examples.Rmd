...
```{r, echo = TRUE, eval=TRUE, fig.height=5, fig.width=10, results = 'show'}
...
# y vs x2
g = ggplot(dat, aes(y = y, x = x2, colour = x1))
g = g + geom_point(colour="grey50", size = 1) + geom_smooth(method = lm, se = FALSE, colour = "black") 
...
```{r, echo=TRUE,  eval=TRUE, fig.height=5, fig.width=10}
...
#### Joe | Fri 05 Nov, 2015
The residuals plot shows that the remaining component (x1) has a slope of -1. The remaining component is the "residuals" by definition of the linear model y ~ x2.

- `ex1 = resid(lm(x1 ~ x2)))` : residuals from removing x2 from x1


```{r, echo = FALSE, eval=FALSE, fig.height=5, fig.width=10}
g2 = ggplot(dat, aes(y = ey, x = x2)) +
    geom_point(colour="grey50", size = 5) + 
    geom_smooth(method = lm, se = FALSE, colour = "black") + 
    geom_point(size = 4) + 
    ggtitle("Residuals of y ~ x2")
g2

g2 = ggplot(dat, aes(y = ex1, x = x2)) +
    geom_point(colour="grey50", size = 5) + 
    geom_smooth(method = lm, se = FALSE, colour = "black") + 
    geom_point(size = 4) + 
    ggtitle("Residuals of x1 ~ x2")
g2
```
...
* Fitted values provide: 1) means for both groups and 2) also provides inference for comparing the two groups ($\beta_1$) using a two-group t-test assuming a common variance.
...
* Heteroscedasticity refers to the circumstance in which the variability of a variable (variance) is unequal across the range of values of a second variable that predicts it.
...
g1 = g
g1 = g1 + geom_abline(intercept = coef(fit)[1], slope = coef(fit)[2], size = 2, colour = "salmon")
g1 = g1 + geom_abline(intercept = coef(fit)[1] + coef(fit)[3], slope = coef(fit)[2], size = 2, colour = "green")
g1
```
