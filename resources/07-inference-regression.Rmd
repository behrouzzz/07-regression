...
# t-statistics for testing H0: Beta0 = 0 and  Beta1 = 0
...
```{r, fig.height=5, fig.width==5, echo=TRUE, results='hide'}
library(ggplot2)
...
* The prediction interval must incorporate the variabilibity
  in the data around the line. This is why there is a "1" in the prediction interval se equation (and not in the confidence interval equation).
  * Even if we knew $\beta_0$ and $\beta_1$ this interval would still have width.
  * This prediction interval will always exist due to noise in the (true) model; the noise $\epsilon_i$ causes $e_i$ to exist, and hence for $\hat{\sigma}$ to exist, which contributes to the prediction interval equation.

---

## In R
```{r, fig.height=5, fig.width=5, echo=TRUE,results='show', eval=TRUE}
# xVals = data.frame(x = seq(min(x), max(x), length = length(x)))
# newdata <- data.frame(x = xVals)

xVals = seq(min(x), max(x), length = length(x))
newdata <- data.frame(x = xVals)
p1 <- predict(fit, newdata, interval = ("confidence"))
p2 <- predict(fit, newdata, interval = ("prediction"))
plot(x, y, frame=FALSE,xlab="Carat",ylab="Dollars",pch=21,col="black", bg="lightblue", cex=2)
abline(fit, lwd = 2)
lines(xVals, p1[,2], col="salmon"); lines(xVals, p1[,3], col="salmon")
lines(xVals, p2[,2], col="lightblue"); lines(xVals, p2[,3], col="lightblue")
