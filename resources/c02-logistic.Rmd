...
```{r linearReg, dependson = "loadRavens", cache=TRUE, fig.width=4, fig.height=4}
lmRavens <- lm(ravensData$ravenWinNum ~ ravensData$ravenScore)
summary(lmRavens)$coef

## Joe | Tue 10 Nov, 2015
require(ggplot2)
g <- ggplot(ravensData, aes(x = ravenScore, y = ravenWinNum)) + 
    geom_smooth(method = "lm") + 
    geom_point() + 
    scale_x_continuous(limits = c(0, max(ravensData$ravenScore)))
g
```
...
## Joe | Tue 10 Nov, 2015
# compare glm and lm -> glm with family=gaussian is same as lm
logRegRavens2 <- glm(ravensData$ravenWinNum ~ ravensData$ravenScore, family="gaussian")
summary(logRegRavens2)

linRegRavens <- lm(ravensData$ravenWinNum ~ ravensData$ravenScore)
summary(linRegRavens)
```
...
- Intercept = 0.1863 is odds of winning with a score of 0.
- Slope = 1.112 is 11% increasing in winning with every 1 point increase in score. So slope is the `odds ratio`.
- Exponentiated confidence interval (0.996229662 1.303304) covers odds of 1, which means variate "ravensScore" is potentially NOT significant.
- NOTE: exp(0) = 1; an odds of 1 -> p = 0.5, which is intepreted as no effect of the regressor on the outcome.
